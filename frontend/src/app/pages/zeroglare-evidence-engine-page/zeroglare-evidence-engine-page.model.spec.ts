...
describe('ZEE page content', () => {
  it('describes the public surface with explicit support policy and replay manifest wording', () => {
    ...
    expect(ZEE_PAGE_CONTENT.hero.anchors.every((anchor) => anchor.route === ZEE_ROUTE_PATH)).toBe(true);
    expect(ZEE_PAGE_CONTENT.hero.anchors.every((anchor) => anchor.fragment.startsWith('pdm-zee-'))).toBe(true);
