...
async function verifyViolationReturnsBlockedReviewState() {
  ...
  assert.equal(body.reviewState.admission, 'blocked', 'violation reviewState admission mismatch.');
  assert.equal(body.rejection, null, 'violation should not expose rejection metadata.');

  process.stdout.write('PASS concept_detail_violation_blocked_review_state\n');
}
...
async function main() {
  await verifyViolationReturnsBlockedReviewState();
