...
const excludedTermSet = new Set(excludedTerms);

const inputPaths = Object.freeze({
  preview: path.join(previewRoot, 'batch_007_resolved_writeback_preview_NOT_APPLIED.json'),
  validationReport: path.join(previewRoot, 'batch_007_resolved_writeback_validation_report.json'),
});
...
const outputPaths = Object.freeze({
  reportMarkdown: path.join(appliedRoot, 'batch_007_resolved_applied_writeback_report.md'),
  reportJson: path.join(appliedRoot, 'batch_007_resolved_applied_writeback_report.json'),
  appliedDiffJson: path.join(appliedRoot, 'batch_007_resolved_applied_diff.json'),
  postApplyValidationReportJson: path.join(
    appliedRoot,
    'batch_007_resolved_post_apply_validation_report.json',
  ),
});
...
function buildCandidates(preview, validationReport, currentSources) {
  assertCondition(preview.status === 'NOT_APPLIED', 'Preview status must be NOT_APPLIED.', preview.status);
  assertCondition(
    preview.finalRecommendation === 'ready_for_apply_prompt',
    'Preview final recommendation must be ready_for_apply_prompt.',
    preview.finalRecommendation,
  );
  assertCondition(
    validationReport.finalRecommendation === 'ready_for_apply_prompt',
    'Validation final recommendation must be ready_for_apply_prompt.',
    validationReport.finalRecommendation,
  );
  assertCondition(preview.candidateCount === expectedCandidateCount, 'Preview candidate count must be 29.');
  assertCondition(preview.addCount === expectedCandidateCount, 'Preview add count must be 29.');
  assertCondition(preview.skipCount === 0, 'Preview skip/blocked count must be 0.');

  const currentGeneratedTerms = new Set(Object.keys(currentSources.terms ?? {}).map(normalizeTerm));
  const candidates = preview.proposedAdditions ?? [];
  assertCondition(candidates.length === expectedCandidateCount, 'Expected exactly 29 proposed additions.');
  ...
  candidates.forEach((candidate) => {
    ...
    if (excludedTermSet.has(candidate.term)) {
      failures.push([candidate.term, 'excluded term included']);
    }
    if (currentGeneratedTerms.has(candidate.normalizedTerm ?? normalizeTerm(candidate.term))) {
...
function supportLabelFor(sourceGroup) {
  if (sourceGroup === 'black') return 'Black primary support';
  if (sourceGroup === 'wharton_v3') return 'Wharton v3 assistive comparator context';
  if (sourceGroup === 'stroud_v3') return 'Stroud v3 assistive comparator context';
  return `${sourceGroup.charAt(0).toUpperCase()}${sourceGroup.slice(1)} comparator context`;
}
...
function sourceRecordFromPointer(candidate, sourceGroup, source) {
  ...
  return {
    ...
    supportNote: `${supportLabelFor(sourceGroup)} for ${candidate.term}: ${support}`,
    snippetDisplay: typeof source.contextPreview === 'string'
      ? source.contextPreview
      : (typeof source.supportingSnippet === 'string' ? source.supportingSnippet : null),
    referenceRole: 'supporting_lexicon_reference',
  };
}

function buildSourceRecords(candidate) {
  return ['black', 'anderson', 'osborn', 'wharton_v3', 'stroud_v3'].flatMap((group) => {
...
function applyMeaningSources(currentSources, candidates, appliedAt) {
  const nextSources = {
    ...
    source: `${currentSources.source}_plus_batch_007_resolved_writeback`,
    terms: {
...
function applyBoundaryOverrides(candidates) {
  ...
  assertCondition(
    ...
    'Candidate semantic override already exists.',
    duplicateTerms.map((candidate) => candidate.term),
  );
  ...
  const updated = original.replace(marker, `${buildOverrideBlock(candidates)}\n${marker}`);
  fs.writeFileSync(targetPaths.vocabularyBoundary, updated, 'utf8');
}

function meaningCountSnapshot() {
  [
    targetPaths.meaningSources,
    targetPaths.vocabularyBoundary,
    path.join(repoRoot, 'backend/src/modules/legal-vocabulary/vocabulary-boundary.js'),
  ].forEach((modulePath) => {
    try {
      delete require.cache[require.resolve(modulePath)];
    } catch {
      // Some paths may not be loaded as modules yet.
    }
  });
...
function buildReport(appliedDiff, postApplyReport) {
  return `${[
    '# Batch 007 Resolved Applied Writeback Report',
    '',
    'Final state: Batch 007 resolved writeback applied.',
    '',
    'Scope: source-backed legal vocabulary meaning layer only. Runtime ontology, concept packets, resolver behavior, aliases, and frontend display were not changed.',
    '',
    '## Summary',
    '',
    ...
    '## Pre-Apply Checks',
    '',
    markdownTable(['Check', 'Result'], appliedDiff.preApplyChecks.map((check) => [
      check.name,
    ...
    '',
    markdownTable(
      ['Term', 'Resolved decision', 'Source basis', 'Black refs', 'Meaning in law'],
      appliedDiff.addedTerms.map((term) => [
        term.term,
        term.resolvedDecision,
        term.sourceBasis,
    ...
    '',
    '## Excluded Terms Preserved',
    '',
    markdownTable(['Term'], appliedDiff.excludedTermsPreserved.map((term) => [term])),
    '',
    '## Validation Commands',
    '',
    markdownTable(['Command', 'Result'], postApplyReport.validationCommandResults.map((result) => [
      result.command,
      result.status,
    ])),
    '',
    ...
    `- No aliases added by Batch 007: ${appliedDiff.noAliasesAdded}`,
    `- inherent_power excluded: ${postApplyReport.safetyChecks.inherentPowerExcluded}`,
...
function main() {
  ...
  const beforeBoundaryText = fs.readFileSync(targetPaths.vocabularyBoundary, 'utf8');
  ...
  precheck('preview_recommendation_ready_for_apply_prompt', preview.finalRecommendation === 'ready_for_apply_prompt', preview.finalRecommendation);
  precheck('validation_report_ready_for_apply_prompt', validationReport.finalRecommendation === 'ready_for_apply_prompt', validationReport.finalRecommendation);
  precheck('preview_candidate_count_29', preview.candidateCount === expectedCandidateCount, preview.candidateCount);
  precheck('preview_add_count_29', preview.addCount === expectedCandidateCount, preview.addCount);
  precheck('preview_blocked_or_skip_count_zero', preview.skipCount === 0, preview.skipCount);

  const candidates = buildCandidates(preview, validationReport, currentSources);
  ...
  const addedBoundaryOverrideText = beforeBoundaryText !== fs.readFileSync(targetPaths.vocabularyBoundary, 'utf8');

  const addedTerms = candidates.map((candidate) => ({
    term: candidate.term,
    normalizedTerm: candidate.normalizedTerm,
    resolvedDecision: candidate.resolvedDecision,
    ...
    whartonV3ComparatorProvenanceCount: candidate.whartonV3ComparatorProvenanceCount,
    stroudV3ComparatorProvenanceCount: candidate.stroudV3ComparatorProvenanceCount,
  }));

  const validationCommandResults = [
    runCommand('node', ['--check', 'backend/scripts/lexicon/writeback/apply-lexicon-writeback-batch-007-resolved.js']),
    runCommand('node', [
      '--test',
  ...
  const countChecks = {
    batch007AddedTermsInSourceCount: candidates.filter((candidate) => (
  ...
  const safetyChecks = {
    inherentPowerExcluded: !Object.hasOwn(afterSources.terms ?? {}, 'inherent_power'),
    excludedTermsPreserved: excludedTerms.every((term) => !Object.hasOwn(afterSources.terms ?? {}, term)),
  ...
  const countsPassed = (
    countChecks.batch007AddedTermsInSourceCount === expectedCandidateCount
  ...
  const finalRecommendation = allCommandsPassed && countsPassed && safetyPassed
    ? 'batch_007_resolved_applied_clean'
  ...
  const appliedDiff = {
    ...
    boundaryHashBefore,
    boundaryHashAfter,
    excludedTermsPreserved: excludedTerms,
    noExistingTermsModified: existingTermsModified.length === 0,
    existingTermsModified,
    noAliasesAdded: candidates.every((candidate) => !candidate.term.includes('_')),
    boundaryOverridesAdded: addedBoundaryOverrideText,
    preApplyChecks,
  };
  ...
  writeJson(outputPaths.reportJson, reportJson);
  ...
  console.log(JSON.stringify({
    ...
    boundaryHashBefore,
    boundaryHashAfter,
    validationPassed: finalRecommendation === 'batch_007_resolved_applied_clean',
    outputPaths,
  }, null, 2));

  if (finalRecommendation !== 'batch_007_resolved_applied_clean') {
    process.exitCode = 1;
  }
}
