...
const outputPaths = Object.freeze({
  previewJson: path.join(outputRoot, 'batch_005_writeback_preview_NOT_APPLIED.json'),
  diffMarkdown: path.join(outputRoot, 'batch_005_writeback_diff_preview.md'),
  validationReportJson: path.join(outputRoot, 'batch_005_writeback_validation_report.json'),
});
...
function buildValidationReport(candidates) {
  ...
  return {
    ...
    scope: 'batch 005 writeback preview only',
    status: 'NOT_APPLIED',
    boundaryDiscipline: {
...
function buildDiffMarkdown(candidates, validationReport) {
  ...
  return [
    ...
    '## Next Step',
    '',
    'If approved, the next step is an explicit live writeback task that applies only these NOT_APPLIED batch 005 candidates and then reruns the boundary audit, vocabulary boundary tests, and boundary contract validation.',
    '',
  ].join('\n');
}
