...
const outputPaths = Object.freeze({
  previewJson: path.join(outputRoot, 'batch_002_writeback_preview_NOT_APPLIED.json'),
  diffMarkdown: path.join(outputRoot, 'batch_002_writeback_diff_preview.md'),
  validationReportJson: path.join(outputRoot, 'batch_002_writeback_validation_report.json'),
});
...
function buildValidationReport(candidates) {
  ...
  return {
    ...
    scope: 'batch 002 writeback preview only',
    status: 'NOT_APPLIED',
    boundaryDiscipline: {
...
function buildDiffMarkdown(candidates, validationReport) {
  ...
  return [
    ...
    '## Alias Caution',
    '',
    'Spaced and underscored alias surfaces are mapped to exact dataset rows where possible. A live apply step must still apply each candidate only to its selected target row and must not fan out to alternate alias rows.',
    '',
    '## Next Step',
    '',
