...
function flattenPack5Corpus() {
  return [
    ...readReviewedClause('legal-floor-protected-person-core.json'),
    ...readReviewedClause('authority-protected-person-core.json'),
    ...readReviewedClause('policy-overlay-protected-person-core.json'),
  ];
}
...
test('Pack 5 reviewed clauses compile into a reference bundle and evaluate deterministically', () => {
  const ajv = buildAjv();
  const sourceRegistry = readModuleJson('fixtures/military-source-registry.json');
  const clauses = flattenPack5Corpus();
  const manifestPath = path.join(MODULE_DIR, 'reference-pack-manifest.protected-person.json');

  assert.equal(readReviewedClause('legal-floor-protected-person-core.json').length, 4);
  assert.equal(readReviewedClause('authority-protected-person-core.json').length, 2);
  assert.equal(readReviewedClause('policy-overlay-protected-person-core.json').length, 2);
  ...
  assert.equal(bundleResult.metadata.bundleId, 'mil-us-protected-person-state-core-bundle');
  ...
  assert.ok(refusalDecision.failingRuleIds.includes('CR-CL-PER-LF-001'));
