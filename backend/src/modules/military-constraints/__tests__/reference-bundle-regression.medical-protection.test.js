...
function buildFactPacket(bundle, packet) {
...
test('Pack 3 reference bundle regression snapshots stay stable', () => {
  const ajv = buildAjv();
  const sourceRegistry = readModuleJson('fixtures/military-source-registry.json');
  const manifest = readModuleJson('reference-pack-manifest.medical-protection.json');
  const corpus = flattenPack3Corpus();
  const snapshots = readJson('reference-expected-decisions.medical-protection.json');
  const packets = readJson('reference-fact-packets.medical-protection.json');

  assert.equal(manifest.packId, 'mil-us-medical-protection-core-v0.1.0');
  ...
  const bundleResult = buildReferenceBundle({
    rootDir: MODULE_DIR,
    manifestPath: path.join(MODULE_DIR, 'reference-pack-manifest.medical-protection.json'),
  });
  ...
  packets.forEach((packet) => {
    ...
    const facts = buildFactPacket(bundleResult.bundle, packet.facts);
