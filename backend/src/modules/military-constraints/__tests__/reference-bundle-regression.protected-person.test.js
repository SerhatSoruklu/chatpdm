...
test('Pack 5 reference bundle regression snapshots stay stable', () => {
  const ajv = buildAjv();
  const sourceRegistry = readModuleJson('fixtures/military-source-registry.json');
  const manifest = readModuleJson('reference-pack-manifest.protected-person.json');
  const corpus = flattenPack5Corpus();
  const snapshots = readJson('reference-expected-decisions.protected-person.json');
  const packets = readJson('reference-fact-packets.protected-person.json');

  assert.equal(manifest.packId, 'mil-us-protected-person-state-core-v0.1.0');
