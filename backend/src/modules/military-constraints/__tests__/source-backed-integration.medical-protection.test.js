...
function flattenPack3Corpus() {
  return [
    ...readReviewedClause('legal-floor-medical-core.json'),
    ...readReviewedClause('authority-medical-core.json'),
    ...readReviewedClause('policy-overlay-medical-core.json'),
  ];
}
...
test('Pack 3 reviewed clauses compile into a reference bundle and evaluate deterministically', () => {
  const ajv = buildAjv();
  const sourceRegistry = readModuleJson('fixtures/military-source-registry.json');
  const clauses = flattenPack3Corpus();
  const manifestPath = path.join(MODULE_DIR, 'reference-pack-manifest.medical-protection.json');

  assert.equal(readReviewedClause('legal-floor-medical-core.json').length, 4);
  assert.equal(readReviewedClause('authority-medical-core.json').length, 2);
  assert.equal(readReviewedClause('policy-overlay-medical-core.json').length, 2);
  ...
  assert.ok(bundleResult.bundle, 'Expected Pack 3 bundle.');
  assert.equal(bundleResult.metadata.packId, 'mil-us-medical-protection-core-v0.1.0');
  assert.equal(bundleResult.metadata.bundleId, 'mil-us-medical-protection-core-bundle');
  ...
  assert.ok(refusalDecision.failingRuleIds.includes('CR-CL-MED-LF-001'));
