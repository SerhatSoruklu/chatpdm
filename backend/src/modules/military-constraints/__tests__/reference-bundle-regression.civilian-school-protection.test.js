...
test('Pack 4 reference bundle regression snapshots stay stable', () => {
  const ajv = buildAjv();
  const sourceRegistry = readModuleJson('fixtures/military-source-registry.json');
  const manifest = readModuleJson('reference-pack-manifest.civilian-school-protection.json');
  const corpus = flattenPack4Corpus();
  const snapshots = readJson('reference-expected-decisions.civilian-school-protection.json');
  const packets = readJson('reference-fact-packets.civilian-school-protection.json');

  assert.equal(manifest.packId, 'mil-us-civilian-school-protection-core-v0.1.0');
  ...
  const bundleResult = buildReferenceBundle({
    rootDir: MODULE_DIR,
    manifestPath: path.join(MODULE_DIR, 'reference-pack-manifest.civilian-school-protection.json'),
  });
