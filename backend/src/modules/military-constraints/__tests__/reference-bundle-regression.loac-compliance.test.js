...
test('US_LOAC_COMPLIANCE_V1 reference bundle regression snapshots stay stable', () => {
  const ajv = buildAjv();
  const sourceRegistry = readModuleJson('fixtures/military-source-registry.json');
  const manifest = readModuleJson('reference-pack-manifest.loac-compliance.json');
  const corpus = [
    ...readReviewedClause('loac-compliance-core.json'),
  ];
  const snapshots = readJson('reference-expected-decisions.loac-compliance.json');
  const packets = readJson('reference-fact-packets.loac-compliance.json');

  assert.equal(manifest.packId, 'US_LOAC_COMPLIANCE_V1');
  ...
  const bundleResult = buildReferenceBundle({
    rootDir: MODULE_DIR,
    manifestPath: path.join(MODULE_DIR, 'reference-pack-manifest.loac-compliance.json'),
  });
