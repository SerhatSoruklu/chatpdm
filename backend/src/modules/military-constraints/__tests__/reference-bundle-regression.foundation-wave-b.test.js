...
const { PACKS } = require('./foundation-wave-b.shared');
...
for (const pack of PACKS) {
  test(`${pack.packId} reference bundle regression snapshots stay stable`, () => {
    ...
    const suffix = pack.manifestFile.slice('reference-pack-manifest'.length, -'.json'.length);
    const snapshots = readJson(`reference-expected-decisions${suffix}.json`);
    const packets = readJson(`reference-fact-packets${suffix}.json`);
