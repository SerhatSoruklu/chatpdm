...
const { PACKS } = require('./overlay-wave-b.shared');
...
PACKS.forEach((pack) => {
  test(`${pack.packId} reviewed clauses compile into a reference bundle and evaluate deterministically`, () => {
    ...
    const clauses = readReviewedClause(pack.clauseFile);
    const manifestPath = path.join(MODULE_DIR, pack.manifestFile);

    assert.equal(clauses.length, pack.expectedClauseCount);
    ...
    const refusalFacts = cloneJson(allowedFacts);
    ...
    assert.deepEqual(refusalDecision.failingRuleIds, pack.expectedRefusal.failingRuleIds);
    assertValid(ajv, 'https://chatpdm.local/schemas/runtime-decision.schema.json', refusalDecision);

    const incompleteFacts = cloneJson(allowedFacts);
