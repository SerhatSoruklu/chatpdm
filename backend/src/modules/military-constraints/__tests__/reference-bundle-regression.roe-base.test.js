...
test('US_RULES_OF_ENGAGEMENT_BASE_V1 reference bundle regression snapshots stay stable', () => {
  const ajv = buildAjv();
  const sourceRegistry = readModuleJson('fixtures/military-source-registry.json');
  const manifest = readModuleJson('reference-pack-manifest.roe-base.json');
  const corpus = [
    ...readReviewedClause('roe-base-core.json'),
  ];
  const snapshots = readJson('reference-expected-decisions.roe-base.json');
  const packets = readJson('reference-fact-packets.roe-base.json');

  assert.equal(manifest.packId, 'US_RULES_OF_ENGAGEMENT_BASE_V1');
  ...
  const bundleResult = buildReferenceBundle({
    rootDir: MODULE_DIR,
    manifestPath: path.join(MODULE_DIR, 'reference-pack-manifest.roe-base.json'),
  });
